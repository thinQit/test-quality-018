...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const toast = useCallback(
    (message: string, typeOrOptions?: ToastType | { variant?: ToastType; type?: ToastType }) => {
      const type =
    ...
    <ToastContext.Provider value={{ toasts, notify, toast, addToast: notify, dismiss }}>
      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2" aria-live="polite">
        {toasts.map(t => (
          ...
          >
            {t.message}
            <button onClick={() => dismiss(t.id)} className="ml-2 font-bold" aria-label="Dismiss notification">
              ×
            </button>
          </div>
        ))}
